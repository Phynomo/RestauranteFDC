...
function PDFDocument() {
  ...
  const doc = new jsPDF();
    ...
    useEffect(() => {
      const fetchData = () => {
          axios.get("api/Platillos/Reporte?id="+JSON.parse(localStorage.getItem('token')).sucu_Id )
            .then(response => {
    ...
    const header = function (data) {
      ...
      // Agregar texto
      doc.text("Reporte de platillos", data.settings.margin.left + 0, 22);
    };
  ...
  doc.autoTable({
    head: [['Id', 'Platillo','Categoria','Precio', 'Pedido']],
    body: data.map((row) => [
      row.plat_Id,
      row.plat_Nombre,
      row.cate_Descripcion,
      row.plat_Precio + " Lps",
      row.cantidadPlatillos + " veces"
    ]),    
