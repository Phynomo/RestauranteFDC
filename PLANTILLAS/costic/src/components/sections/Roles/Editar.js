...
function Content() {
  ...
  useEffect(() => {
    console.log("*gemidos*");
    console.log(selectedRoles);
    fetch('/api/Pantallas/PantallasPorRol')
      .then(response => response.json())
      .then(data => {
        const selectedRoless = data.data.map(item => {
          return {
            id: item.prol_Id,
            prol_Id: item.prol_Id,
            role_Id: item.role_Id,
            pant_Id: item.pant_Id,
          }
        });
        setSelectedRoles(selectedRoless.map(option => ({ value: option.pant_Id, label: option.pant_Nombre})));
        // setIsLoading(false);
      })
      .catch(error => {
        console.log("Error en la solicitud fetch:", error);
