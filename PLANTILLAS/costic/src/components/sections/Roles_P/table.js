...
const DataTable = () => {
  ...
  .then(response => {
      ...
      if (response.data.message == "Registro eliminado") {
          ...
          handleClose();
      } else if(response.data.message == "enuso")
      {
        alertError("Error", "El registro no se elimino porque esta en uso", "2000");
        handleClose();
      }else {
  ...
  const columns = [
    ...
    { field: 'role_Nombre', headerName: 'Rol', flex: 2 },
    {
      field: 'actions',
      headerName: 'Acciones',
      sortable: false,
      ...
      renderCell: (params) => (
        <div className=''>
          ...
          <a style={{ margin: "5px" }} onClick={() => seleccionarUsuario(params.row.role_Id)} ><i class='fas fa-trash-alt text-danger'></i></a>
        </div>
      ),
    },
  ];
      ...
      .then(response => {
        const rows = response.data.data.map(item => {
          return {
            ...
            user_FechaCreacion: item.role_FechaCreacion,
            user_NombreUsuModificacion: item.user_NombreUsuModificacion,
            user_FechaModificacion: item.role_FechaModificacion,
          };
        });
        setRows(rows);
        setIsLoading(false);
