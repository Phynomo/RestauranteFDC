...
function PDFDocument() {
  ...
  const doc = new jsPDF();
    ...
    useEffect(() => {
      const fetchData = () => {
          axios.get("api/Empleados/Reporte?id="+JSON.parse(localStorage.getItem('token')).sucu_Id )
            .then(response => {
    ...
    const header = function (data) {
      ...
      // Agregar texto
      doc.text("Reporte de empleados", data.settings.margin.left + 0, 22);
    };
      ...
      const footer = function (data) {
        ...
        const text = `Documento generado por ${JSON.parse(localStorage.getItem('token')).user_NombreUsuario} el ${date}`;
  ...
  doc.autoTable({
    head: [['Id', 'Nombre completo','Indentidad','Fecha Nacimiento', 'Sexo', 'recidencia','Cargo']],
    body: data.map((row) => [
      row.empe_Id,
      row.empe_NombreCompleto,
      row.empe_Identidad,
      new Date(row.empe_FechaNacimiento).toLocaleDateString('es-ES', {day: '2-digit', month: '2-digit', year: 'numeric'}),
      row.empe_Sexo,
      row.muni_Nombre,
      row.carg_Descripcion
    ]),    
